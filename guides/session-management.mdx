...
Sessions are stored in a platform-appropriate user data directory resolved by [`platformdirs`](https://platformdirs.readthedocs.io/). You can override the base directory with the `ARCHEO_DATA_DIR` environment variable:

```bash
# Use the default platform data directory
uv run archeo-cluster sessions
# Storage location: /home/user/.local/share/archeo-cluster  (Linux example)
...
<Note>
  On macOS the default base directory is `~/Library/Application Support/archeo-cluster`. On Windows it is `%APPDATA%\archeo-cluster`.
</Note>
